...
function Color({ id, title, color, rating, callbackRateColor, callbackRemoveColor }) {
  // 이벤트 핸들러 작성.
  const handlerRemove = () => {
  ...
  return (
    <section className="color">
      <h1>{title}</h1>
      <button onClick={handlerRemove}>X</button> <div className="color" style={{ backgroundColor: color }}></div>
      <div>
        <StarRating starsSelected={rating} handlerRate={handlerRate}></StarRating>
      </div>
    </section>
  );
}
